...
export const SpentItem =({game_name, price, amount_currency, className}:{game_name?:string, price?:string, amount_currency?:string, className?:string})=>{
    return (
        <div className={`${className} h-44 bg-black rounded`}>
...
export const EarntSection = ({ inventoryOpen, data }: { inventoryOpen: Number, data?:CoinsEarned[] }) => {
  return (
    <div className={`${inventoryOpen !== 3 ? "flex" : "hidden"}`}>
      <div className="flex flex-col w-full h-[40dvh] overflow-y-scroll scrollbar">
        {
            data && data.map((item)=>{
